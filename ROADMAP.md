# 学习路线：前沿 LLM 端到端训练知识与工程能力

版本：课程 v0.1，2026-09-08。目标是系统学习前沿 LLM 的端到端训练知识，并建立、提升相应的工程能力。面向初学者及希望进阶的工程师、研究者，尤其是没有 frontier lab 经历的学习者。默认入口是会写 Python、尚无大模型训练经验；已有基础可通过诊断进入后续模块。
...
知识主线贯穿目标与预算、数据、架构、预训练与阶段训练、后训练、推理、评估和发布；工程主线贯穿实现、正确性、可复现性、性能与可靠性。二者在每个模块中共同推进。

| 层级 | 端到端训练知识 | 工程能力 | 可检查的学习产物 |
|---|---|---|---|
| 建立基础 | 解释 token、模型、loss、优化、数据与评估如何形成训练闭环 | 实现关键算子和小训练循环，检查梯度、mask 与数据，记录可复现环境 | 可运行的小实现、数值检查与误差解释；主要由 M00–M04 建立 |
| 深化能力 | 理解规模、并行、MoE、数值精度及后训练目标的设计取舍 | 追踪真实代码，设计对照，定位数值/数据/系统问题，验证性能和恢复 | 源码证据、故障分析、测量或明确标注范围的机制实验；主要在 M05–M14 展开 |
| 综合运用 | 为具体目标组织从数据到评估的完整方案，并解释上下游影响 | 在明确资源与版本条件下实现或审计一条链路，复核结果并提出有证据的改进 | M15 综合项目：配置、代码或审计证据、实验记录、评估与限制 |

这些是学习目标，当前教材与实验的完成情况另见 [覆盖地图](COVERAGE.md)。CPU 练习可以证明部分机制和实现的正确性；真实 GPU 性能、集群可靠性与大规模训练能力需要对应环境下的证据。
...
| [M12 Harness、任务与轨迹](curriculum/03-posttraining-and-agents.md#m12) | 模型实际看见、生成和完成了什么？ | Harbor/Verifiers、Pi/SoL-Pi/DeepSeek Harness、Claude Code | 环境与评分契约、带来源的 token trace |
...
现有 21 个项目仍是主干，登记编号保留用于稳定索引；**编号不是学习顺序**。每个模块先选一份主实现读透，再与另一份比较，避免在六种 RL 框架之间跳转却无法解释一次更新。
