...
export function updateTokenResults(root,result,target,lastStep,history){
  ...
  if(lastStep){const delta=lastStep.after-lastStep.before;feedback.innerHTML=`<small>JUST UPDATED / 刚才这一步</small><div class="step-loss"><span>${fixed(lastStep.before)}</span><span>→</span><strong>${fixed(lastStep.after)}</strong></div><div class="step-delta">Loss ${delta<=0?'降低':'升高'} ${fixed(Math.abs(delta))}</div>`;}
  const chart=root.querySelector('#loss-history');chart.hidden=history.length<2;
  if(history.length>1)chart.innerHTML=`<header><span>本次参数更新轨迹</span><span>${history.length-1} 次更新</span></header><svg viewBox="0 0 340 82" role="img" aria-label="Loss 从 ${escapeText(fixed(history[0]))} 变为 ${escapeText(fixed(history.at(-1)))}"><line class="chart-baseline" x1="8" y1="74" x2="332" y2="74"/><polyline points="${lossHistoryPoints(history)}"/></svg><p>纵轴自适应：${fixed(Math.min(...history),4)}–${fixed(Math.max(...history),4)}。修改输入条件后重新记录。</p>`;
}

export function batchResultsHtml(batch,masks){
  if(!batch.count)return '<div class="notice warning">没有有效目标，无法定义平均 loss。请至少勾选一个目标；不能把空批次的 loss 记作 0。</div>';
  const different=Math.abs(batch.global.loss-batch.equalGroups.loss)>1e-8;
  return `<div class="batch-groups">${batch.groups.map((group,i)=>`<div class="batch-group"><header><span>GROUP ${i+1}</span><b>${group.count} 个有效目标</b></header><div class="batch-tokens">${group.ids.map(id=>`<span class="batch-token ${masks[id]?'':'masked'}" aria-label="位置 ${id+1}，标签 ${['A','C','C','C','B','A'][id]}，${masks[id]?'计入 loss':'已屏蔽目标'}">${['A','C','C','C','B','A'][id]}</span>`).join('')}</div></div>`).join('')}</div><div class="aggregation-comparison"><div><small>全局 token 平均</small><strong>${fixed(batch.global.loss)}</strong></div><span>${different?'≠':'='}</span><div><small>局部均值直接平均</small><strong class="${different?'different':''}">${fixed(batch.equalGroups.loss)}</strong></div></div><p class="small-text muted">有效目标共 ${batch.count} 个，两组 ${batch.groups.map(g=>g.count).join(' : ')}。${batch.groups.some(g=>!g.count)?'空组不参与平均，贡献为 0。':''}</p><details class="precision-details"><summary>查看三种聚合的 loss 与梯度</summary><div class="table-scroll"><table class="data-table"><thead><tr><th>聚合规则</th><th>Loss</th><th>梯度 [A, B, C]</th></tr></thead><tbody>${[['全局 token 平均',batch.global],['按有效数量加权',batch.weighted],['局部均值直接平均',batch.equalGroups]].map(([label,value])=>`<tr><td>${label}</td><td class="mono">${fixed(value.loss)}</td><td class="mono">[${value.gradient.map(g=>fixed(g,4)).join(', ')}]</td></tr>`).join('')}</tbody></table></div></details><div class="math-line">L = Σ (n_group × L_group) / Σ n_group</div><p class="${different?'error':'good'} small-text">${different?'本例中，直接平均局部均值改变了目标。':'当前两种 loss 相同；是否是同一目标，还需比较权重与梯度。'}</p>`;
}
