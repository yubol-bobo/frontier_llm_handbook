...
- 顶部选择中文 / English，切换界面和全部 54 篇站内资料。显式 `?lang=zh` / `?lang=en` 链接优先于浏览器保存的语言偏好。
...
双语检查还覆盖 54 篇译文的章节、列表、表格、引用与数值对应、所有旧锚点、界面翻译覆盖和个人输入不被翻译。构建输出 `content.json` 与 `content.en.json`，分别按语言加载；英文由 `translations/en/` 的完整 Markdown 生成，源文件哈希不匹配时构建会失败。修改文档后按 [双语维护说明](../translations/README.md) 核对英文版本。

已有完整 upstream clones 的维护者另运行 `python tools/validate_learning_repo.py`。CI 不下载 20 个上游仓库，只检查网站依赖的资料及生成结果。
