...
- 顶部选择中文 / English，切换界面和全部 57 篇站内资料。显式 `?lang=zh` / `?lang=en` 链接优先于浏览器保存的语言偏好。
...
主题与语言偏好分别保存，升级沿用原有学习记录格式。切换语言保留当前页面、笔记、尚未确认的状态选择、实验参数、自测作答和阅读位置。交互支持键盘焦点和减少动态效果偏好；切换主题会更新文档关系图，不重建笔记输入区域。
...
双语检查还覆盖 57 篇译文的章节、列表、表格、引用与数值对应、所有旧锚点、界面翻译覆盖和个人输入不被翻译。构建输出 `content.json` 与 `content.en.json`，分别按语言加载；英文由 `translations/en/` 的完整 Markdown 生成，源文件哈希不匹配时构建会失败。修改文档后按 [双语维护说明](../translations/README.md) 核对英文版本。

已有完整 upstream clones 的维护者另运行 `python tools/validate_learning_repo.py`。CI 不下载 21 个上游仓库，只检查网站依赖的资料及生成结果。
...
- `i18n.mjs` / `locales/`：界面语言、英文文案和元数据；只翻译编写好的文案，不改写插入的个人数据。
