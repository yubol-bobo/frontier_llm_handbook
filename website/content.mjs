...
export const connections = [
  ...
  { id:'recovery', title:'张量放在哪里，决定怎样恢复训练', modules:['M05','M08','M09'], text:'并行布局决定参数与优化器状态的所有权。恢复还需要 master 权重、随机状态、数据位置与完整提交条件。', evidence:'固定源码与公开案例综合，未验证不同拓扑恢复逐位一致。', source:'handbook/02-distributed-pretraining-operations.md' },
  ...
  { id:'reward', title:'任务反馈，怎样成为训练目标', modules:['M11','M12','M13','M14'], text:'环境反馈需要明确的奖励维度与标量化规则。轨迹同时保留动作 token、logprob、mask、分支及版本，才能进入正确的更新。', evidence:'奖励契约已有局部 CPU 实验，完整训练集成待验证。', source:'handbook/03-posttraining-agent-rl-evaluation.md' },
...
export async function buildContent(root) {
  ...
  for(const file of files) {
    const markdown=(await fs.readFile(path.join(root,file),'utf8')).replace(/\r\n/g,'\n');
    const title=markdown.match(/^# (.+)$/m)?.[1]||file;
    const {html,headings}=renderer(file,documentPaths,snapshots).render(markdown);
    docs[file]={path:file,title,html,headings,markdown,minutes:Math.max(1,Math.ceil(markdown.length/650))};
  }
  const modules=[];
  ...
  return {schemaVersion:1,repoUrl,stages,modules,seminars,resources,connections,prerequisites,docs};
}
