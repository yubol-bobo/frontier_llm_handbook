  ...
  try{
    ...
    for(let i=0;i<nodes.length;i++){
      ...
      try{
        ...
        const diagram=document.createElement('div');diagram.className='mermaid';diagram.dataset.source=source;diagram.innerHTML=svg;diagram.setAttribute('role','img');diagram.setAttribute('aria-label','文档关系图，展开下方文字可阅读图的原始关系');
      ...
      }catch{if(obsolete())return;if(nodes[i].tagName==='PRE'){nodes[i].classList.remove('mermaid');nodes[i].setAttribute('aria-label','关系图文字源，图形暂无法显示');}}
    }
  }catch{if(!obsolete())toast('关系图暂时无法加载，已保留文字关系。');}
  };
...
async function navigate(){
  ...
  if(!matchMedia('(prefers-reduced-motion: reduce)').matches)main.animate([{opacity:.55,transform:'translateY(7px)'},{opacity:1,transform:'translateY(0)'}],{duration:260,easing:'cubic-bezier(.22,1,.36,1)'});
...
main.addEventListener('click',async event=>{
  ...
  if(action==='step'){
    try{const before=tokenStep(logits,target,rate);if(before.next.some(v=>Math.abs(v)>10000)){toast('下一步超出输入范围，请先调整 logits 或降低学习率。');return;}if(!lossHistory.length)lossHistory.push(before.loss);lossHistory.push(before.nextLoss);lastStep={before:before.loss,after:before.nextLoss,probabilities:[...before.probabilities]};logits=before.next;syncLogitInputs();updateLab();announceLab('完成一次更新，loss 从 '+fixed(before.loss)+' 变为 '+fixed(before.nextLoss));}catch(e){toast(e.message);}
  }
  if(action==='shift'){
...
main.addEventListener('change',async event=>{
  ...
  if(el.dataset.mask!==undefined){masks[Number(el.dataset.mask)]=Number(el.checked);updateLab();announceLab('当前有 '+masks.reduce((a,b)=>a+b,0)+' 个有效目标。');}
  if(el.id==='partition'){partition=el.value;updateLab();announceLab('已切换分组，结果已更新。');}
...
mountIcons();
initializeTheme(()=>{void renderMermaid(routeVersion);});
