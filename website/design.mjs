...
export function renderLearningView({data,progress,entry,readLink,moduleLink,statusTag,stageFilter}){
  ...
  return `<div class="page-heading learn-heading"><div><div class="eyebrow"><span class="eyebrow-dot"></span> THE FRONTIER IS OPEN</div><h1>理解原理。<span>构建前沿。</span></h1><p>从第一枚 token，到完整训练系统。<br class="mobile-break">一条有源码、有实践、有连接的学习路径。</p></div><a class="text-link" href="${readLink('ROADMAP.md')}">完整路线与入门诊断 ${icon('arrow-up-right')}</a></div>
  <div class="learn-overview"><section class="continue-card"><div class="continue-content"><div class="eyebrow">${icon('play')}${started?'CONTINUE LEARNING / 继续学习':'YOUR FIRST STEP / 从这里开始'}</div><span class="continue-module">${next.id}<span>CORE MODULE</span></span><h2>${escapeText(next.title)}</h2><p>${escapeText(next.deliverable)}</p><a class="button primary" href="${moduleLink(next.id)}">${started?'继续探索':'进入学习'} ${icon('arrow')}</a></div><div class="continue-route" aria-label="三个学习阶段">${data.stages.map((s,i)=>`<a class="mini-route ${next.stage===s.id?'current':''}" href="${moduleLink(s.ids[0])}"><span class="mini-route-dot">${next.stage===s.id?icon('play'):'0'+(i+1)}</span><span><small>${stageNames[i]}</small><strong>${s.name}</strong></span>${icon('chevron')}</a>`).join('')}</div></section>
  ...
  <div class="learning-meta"><span><strong>16</strong> 核心模块</span><span><strong>06</strong> 前沿专题</span><span><strong>19</strong> 固定源码项目</span><span class="meta-note">${icon('code')} 普通电脑即可开始</span></div>
